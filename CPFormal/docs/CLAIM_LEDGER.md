# Ledger de afirmacoes — checkpoint 0.22.0 Genuine-first
...
| GREEN-FIN-SIGNED | o fluxo Green real finito orientado satisfaz `flux=2*delta*finiteRadialGreenEnergy+boundary`, com todos os termos definidos explicitamente | KERNEL_CHECKED | GREEN-FIN-STOKES, GREEN-PHASE-RADIAL, GREEN-RADIAL-COFACTOR | usar a positividade finita e ligar o bordo ao bracket Genuine |
...
| GREEN-CERT | existe um `SignedGreenCertificate` concreto para a carta Genuine | OPEN_BRIDGE | GREEN-FIN-SIGNED e GREEN-FIN-ENERGY-POS estao fechados; faltam anulacao do fluxo em zeros, identificacao/cancelamento do endpoint interno pelo traco e controle do limite | ligar o bracket Genuine ao fluxo e ao bordo sem absorver o endpoint por definicao |
